                ...
                <?php
                    if( $_SERVER['REQUEST_METHOD'] == 'POST' ){
                        ...
                        if( empty($user) || empty($pass)){
                            echo "<div class=\"error\">Write username and password to login.</div>";
                        }
                        else{
                            $get_user_data = $conn -> query("SELECT * 
                            FROM users WHERE user_username = '$user'
                            AND user_password = '$pass'") -> fetchAll(PDO::FETCH_ASSOC);
                            if( count($get_user_data) > 0 ){
                                if(  $get_user_data[0]['user_usertype'] ==  1 ){
                                    $_SESSION['user'] = $user;
                                    $_SESSION['full'] = $get_user_data[0]['user_fullname'];
                                    $_SESSION['usertype'] = 'admin';
                                    $_SESSION['userid'] = $get_user_data[0]['user_userid'];
                                    // header("location:pages/admin.php");
                                    // echo " <script> alert('Welcome Dear $_SESSION[full] Your Role Is Admin');</script>";
                                    ...
                                    // header( "Refresh: 5s;" );
                                    // echo " <script> alert('Welcome Dear $_SESSION[full] Your Role Is User');</script>";
                                    header("Refresh:0; url=pages/students.php");
                                    // exit;
                                }
                            }
...
<html lang="en">
  <head>
    ...
    <link
      ...
      rel="stylesheet"
    />
    <link rel="stylesheet" href="./master/build/css/tailwind.css" />
